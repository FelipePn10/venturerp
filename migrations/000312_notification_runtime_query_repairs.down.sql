...
-- Restore the function body installed by 000308. PostgreSQL resolves the stale
-- column only when the function executes, so rollback remains schema-reversible.
...
BEGIN
    SELECT id,enterprise_id,business_code,pdm_description_technique,engineering_item_base_cod,nature,created_by INTO item_row FROM items WHERE code=p_item_code;
