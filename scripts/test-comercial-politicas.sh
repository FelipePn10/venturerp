...
SMOKE_POLICY_SEQUENCE="${SMOKE_POLICY_SEQUENCE:-$$}"
...
policy_body=$(printf '{"description":"Desconto volume smoke","kind":"DISCOUNT","choice_type":"INFORMATION","calc_type":"PERCENT","percent_value":10,"min_quantity":5,"priority":1,"sequence":%s,"stackable":true,"allow_manual_change":true,"used_in_commission":true,"applies_to_items":true,"data_types_json":["ITEM","CUSTOMER","PAYMENT_TERM"],"rule_json":{"source":"smoke"}}' "$SMOKE_POLICY_SEQUENCE")
created=$(curl_json POST /api/customers/support/commercial-policies/ "$policy_body")
...
curl_json POST "/api/customers/support/commercial-policies/${code}/lines" '{
  ...
  "min_value":0,
  "max_value":1000,
  "variables_json":{"item_code":"10001"}
}' >/dev/null
