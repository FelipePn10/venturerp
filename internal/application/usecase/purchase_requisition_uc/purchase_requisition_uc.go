...
func NewPurchaseRequisitionUseCase(repo repository.PurchaseRequisitionRepository, auth ...ports.AuthService) *PurchaseRequisitionUseCase {
	uc := &PurchaseRequisitionUseCase{repo: repo}
	if len(auth) > 0 {
		uc.auth = auth[0]
	}
	return uc
