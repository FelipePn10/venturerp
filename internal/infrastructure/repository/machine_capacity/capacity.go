...
func Hours(ctx context.Context, pool *pgxpool.Pool, workCenterID int64, day *time.Time) (float64, error) {
 ...
 SELECT w.id,CASE WHEN EXISTS(SELECT 1 FROM maintenance_orders o LEFT JOIN maintenance_plans p ON p.id=o.plan_id WHERE COALESCE(o.machine_id,p.machine_id)=w.id AND o.scheduled_date=$3::date AND o.is_active AND o.status IN ('PLANNED','IN_PROGRESS')) THEN '{}'::tsmultirange ELSE w.windows-COALESCE((SELECT range_agg(tsrange(d.starts_at::timestamp,COALESCE(d.ends_at,NOW())::timestamp,'[)')) FROM machine_downtimes d WHERE d.machine_id=w.id AND d.enterprise_id=$2 AND d.starts_at<$3::date+INTERVAL '2 days' AND COALESCE(d.ends_at,NOW())>$3::date),'{}'::tsmultirange) END AS windows FROM machine_windows w
