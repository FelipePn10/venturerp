...
func (r *DeliveryPromiseParamsRepositorySQLC) Get(ctx context.Context) (*entity.DeliveryPromiseParams, error) {
...
func (r *DeliveryPromiseParamsRepositorySQLC) Save(ctx context.Context, p *entity.DeliveryPromiseParams) (*entity.DeliveryPromiseParams, error) {
	existing, err := r.q.GetDeliveryPromiseParams(ctx)
	if err != nil || existing.ID == 0 {
		row, err := r.q.UpsertDeliveryPromiseParams(ctx, sqlc.UpsertDeliveryPromiseParamsParams{
	...
	row, err := r.q.UpdateDeliveryPromiseParams(ctx, sqlc.UpdateDeliveryPromiseParamsParams{
		UseDeliveryPromise:      p.UseDeliveryPromise,
		BlockedOrdersInPromise:  p.BlockedOrdersInPromise,
		DefaultOrderSort:        p.DefaultOrderSort,
		ShowOrderValues:         int32(p.ShowOrderValues),
...
func rowToEntity(row sqlc.DeliveryPromiseParam) *entity.DeliveryPromiseParams {
