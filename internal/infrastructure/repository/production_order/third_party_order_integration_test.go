...
func TestManualProductionOrderCreatesExternalServiceOrderAtomically(t *testing.T) {
	...
	actor := uuid.New()
	if _, err := pool.Exec(ctx, `INSERT INTO items(code,business_code,warehouse_code,warehouse_unit_of_measurement,created_by,enterprise_id) VALUES($1,($1::bigint)::text,$1,'UN',$2,$3)`, itemCode, actor, enterpriseID); err != nil {
		t.Fatal(err)
	}
	var operationID, routeID int64
	if err := pool.QueryRow(ctx, `INSERT INTO operations(code,name,origin,created_by,lead_time_days,third_party_remittance,enterprise_id) VALUES($1,'External manual OF','TERCEIROS',$2,3,'ORDER_ITEM',$3) RETURNING id`, operationCode, actor, enterpriseID).Scan(&operationID); err != nil {
		t.Fatal(err)
	}
	if err := pool.QueryRow(ctx, `INSERT INTO manufacturing_routes(code,item_code,alternative,is_standard,is_active,created_by,enterprise_id) VALUES($1,$2,1,TRUE,TRUE,$3,$4) RETURNING id`, routeCode, itemCode, actor, enterpriseID).Scan(&routeID); err != nil {
		t.Fatal(err)
	}
