...
const subtractWorkdaysTenantSQL = `SELECT subtract_workdays($1::date, $2::int, $3::bigint)`
...
func (q *Queries) SubtractWorkdaysForEnterprise(ctx context.Context, from time.Time, days int, enterpriseID int64) (time.Time, error) {
	fromDate := pgtype.Date{Time: from.UTC().Truncate(24 * time.Hour), Valid: true}
	var result pgtype.Date
	err := q.db.QueryRow(ctx, subtractWorkdaysTenantSQL, fromDate, int32(days), enterpriseID).Scan(&result)
	if err != nil {
		return time.Time{}, err
	}
	return result.Time, nil
