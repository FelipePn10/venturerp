...
WITH RECURSIVE bom_tree AS (
    SELECT parent_code, child_code, quantity, loss_percentage, parent_mask, is_coproduct, is_fixed_qty, substitute_group, substitute_priority, start_date, end_date
    FROM item_structures
    WHERE parent_code = ANY($1::bigint[]) AND is_active = TRUE

    UNION

    SELECT s.parent_code, s.child_code, s.quantity, s.loss_percentage, s.parent_mask, s.is_coproduct, s.is_fixed_qty, s.substitute_group, s.substitute_priority, s.start_date, s.end_date
    FROM item_structures s
    INNER JOIN bom_tree bt ON s.parent_code = bt.child_code
    WHERE s.is_active = TRUE
)
SELECT parent_code, child_code, quantity, loss_percentage, parent_mask, is_coproduct, is_fixed_qty, substitute_group, substitute_priority, start_date, end_date FROM bom_tree
`
